...
namespace winforms_3
{
    public partial class BrandTable : Form
    {
        ...
        private void DataGridViewTruck_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            (Loader.carBrandDictionaryList[carBrand][selectedCar] as Truck).m_wheels = int.Parse(row.Cells[5].Value.ToString());
            (Loader.carBrandDictionaryList[carBrand][selectedCar] as Truck).m_trunkVolume = int.Parse(row.Cells[6].Value.ToString());
        }

        private void DataGridViewPassenger_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            int selectedCar = e.RowIndex + dataGridViewTruck.Rows.Count;
            DataGridViewRow row = dataGridViewPassenger.Rows[selectedCar];
            ...
            Loader.carBrandDictionaryList[carBrand][selectedCar].m_maxSpeed = int.Parse(row.Cells[3].Value.ToString());
            Loader.carBrandDictionaryList[carBrand][selectedCar].m_licencePlate = row.Cells[4].Value.ToString();
            (Loader.carBrandDictionaryList[carBrand][selectedCar] as PassengerCar).m_multimedia = row.Cells[5].Value.ToString();
            (Loader.carBrandDictionaryList[carBrand][selectedCar] as PassengerCar).m_airbags = int.Parse(row.Cells[6].Value.ToString());
        }
        ...
        private void ReceiveTableData(string BrandTalbeData)
        {
            ...
            int senderPort = int.Parse(carsData[0]);
